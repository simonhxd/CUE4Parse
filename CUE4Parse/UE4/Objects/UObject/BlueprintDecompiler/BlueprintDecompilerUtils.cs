...
public static class BlueprintDecompilerUtils
{
        ...
        string parameters)
    {
        if (className.StartsWith("SolarisMathLibrary_") || className == "KismetMathLibrary")
        {
            ...
            if (functionName.StartsWith("NotEqual_"))
                return $"({parametersList[0]} != {parametersList[1]})";
            if (functionName.StartsWith("NotEqualExactly_"))
                return $"({parametersList[0]} != {parametersList[1]})";
            if (functionName.StartsWith("LessEqual_"))
                return $"({parametersList[0]} <= {parametersList[1]})";
            if (functionName.StartsWith("Less_"))
                return $"({parametersList[0]} < {parametersList[1]})";
            if (functionName.StartsWith("GreaterEqual_"))
                return $"({parametersList[0]} >= {parametersList[1]})";
            if (functionName.StartsWith("Greater_"))
                return $"({parametersList[0]} > {parametersList[1]})";

            if (functionName.StartsWith("Add_"))
                return $"{parametersList[0]} + {parametersList[1]}";
            if (functionName.StartsWith("Xor_"))
                return $"({parametersList[0]} ^ {parametersList[1]})";
            if (functionName.StartsWith("Multiply_"))
                return $"({parametersList[0]} * {parametersList[1]})";
            if (functionName.StartsWith("Percent_"))
                return $"({parametersList[0]} % {parametersList[1]})";
            if (functionName.StartsWith("Or_"))
                return $"({parametersList[0]} | {parametersList[1]})";
            if (functionName.StartsWith("Subtract_"))
                return $"{parametersList[0]} - {parametersList[1]}";
            if (functionName.StartsWith("Not_PreBool"))
                return $"!{parametersList[0]}";
            if (functionName.StartsWith("Not_"))
                return $"(~{parametersList[0]})";
            if (functionName.StartsWith("Select"))
                return $"({parametersList[2]} ? {parametersList[0]} : {parametersList[1]})";
            if (functionName.StartsWith("AddEquals"))
                return $"({parametersList[0]} += {parametersList[1]})";
            if (functionName.StartsWith("Subtract"))
                return $"({parametersList[0]} - {parametersList[1]})";
            if (functionName.StartsWith("Divide"))
                return $"({parametersList[0]} / {parametersList[1]})";
            if (functionName.StartsWith("MultiplyByPi"))
                return $"({parametersList[0]} * π)";
            if (functionName.StartsWith("Multiply"))
                return $"({parametersList[0]} * {parametersList[1]})";
            if (functionName.StartsWith("BooleanAND"))
                return $"{parametersList[0]} && {parametersList[1]}";
            if (functionName.StartsWith("BooleanNAND"))
                return $"!({parametersList[0]} && {parametersList[1]})";
            if (functionName.StartsWith("BooleanOR"))
                return $"({parametersList[0]} || {parametersList[1]})";
            if (functionName.StartsWith("BooleanXOR"))
                return $"{parametersList[0]} ^ {parametersList[1]}";
            if (functionName.StartsWith("BooleanNOR"))
                return $"!({parametersList[0]} || {parametersList[1]})";
            if (functionName.StartsWith("Floor"))
                return $"Floor({parametersList[0]})";
            if (functionName.StartsWith("Abs"))
                return $"{parametersList[0]} < 0.0 ? -{parametersList[0]} : {parametersList[0]}";
            if (functionName.StartsWith("CheckConstrainedFloat"))
            ...
            if (functionName.StartsWith("Negate"))
                return $"-{parametersList[0]}";
            if (functionName.StartsWith("Ceil"))
                return $"Ceil({parametersList[0]})";

            if (functionName.StartsWith("UncheckedConvertI32I64"))
                return $"{parametersList[0]}";
            if (functionName.StartsWith("MakeTransform"))
                return $"FTransform({parametersList[0]}, {parametersList[1]}, {parametersList[2]})";
            if (functionName.StartsWith("Conv_VectorToTransform"))
                return $"FTransform({parametersList[0]})";
            if (functionName.StartsWith("MakeVector2D"))
                return $"FVector({parametersList[0]}, {parametersList[1]})";
            if (functionName.StartsWith("MakeVector"))
                return $"FVector({parametersList[0]}, {parametersList[1]}, {parametersList[2]})";
            if (functionName.EndsWith("ToVector"))
                return $"FVector((float){parametersList[0]})";
            if (functionName.StartsWith("MakeRotator"))
                return $"FRotator({parametersList[0]}, {parametersList[1]}, {parametersList[2]})";
            if (functionName.StartsWith("MakeTimespan"))
                return $"FTimespan({parametersList[0]}, {parametersList[1]}, {parametersList[2]}, {parametersList[4]} * 1000 * 1000)";
            if (functionName.StartsWith("MakeColor"))
                return $"FLinearColor({parametersList[0]}, {parametersList[1]}, {parametersList[2]}, {parametersList[3]})";
            if (functionName.StartsWith("ComposeRotators"))
                return $"FRotator(FQuat({parametersList[0]}) * FQuat({parametersList[1]}))";
            if (functionName.EndsWith("ToLinearColor"))
                return $"FLinearColor({parametersList[0]})";

            if (functionName.StartsWith("Conv_IntToBool"))
                return $"({parametersList[0]} != 0)";
            if (functionName.StartsWith("Conv_BoolToInt"))
                return $"({parametersList[0]} ? 1 : 0)";
            if (functionName.StartsWith("Conv_BoolToByte"))
                return $"({parametersList[0]} ? 1 : 0)";
            if (functionName.StartsWith("Conv_BoolToFloat"))
                return $"({parametersList[0]} ? 1.0f : 0.0f)";
            if (functionName.StartsWith("Conv_BoolToDouble"))
                return $"({parametersList[0]} ? 1.0 : 0.0)";

            if (functionName.EndsWith("ToDouble"))
                return $"((double){parametersList[0]})";
            if (functionName.EndsWith("ToFloat"))
                return $"((float){parametersList[0]})";
            if (functionName.EndsWith("ToInt64"))
                return $"((int64){parametersList[0]})";
            if (functionName.EndsWith("ToInt"))
                return $"((int32){parametersList[0]})";
            if (functionName.EndsWith("ToByte"))
...
{parametersList[4]} = {parametersList[0]}.A";
        ...
        if (className == "KismetStringLibrary")
        {
            ...
            if (functionName.StartsWith("Conv_BoolToString"))
                return $"{parametersList[0]} ? \"true\" : \"false\"";
            if (functionName.EndsWith("ToString"))
                return $"FString({parametersList[0]})";
            if (functionName.EndsWith("ToName"))
                return $"FName({parametersList[0]})";
            if (functionName.StartsWith("Concat_StrStr"))
                return string.Join(" + ", parametersList);
            if (functionName.StartsWith("ParseIntoArray"))
                return $"{parametersList[0]}.Split({parametersList[1]}, /* removeEmpty = */ {parametersList[2]})";
            if (functionName.StartsWith("Contains"))
                return $"{parametersList[0]}.Contains({parametersList[1]}, /* removeEmpty = */ {parametersList[2]})";
            if (functionName.StartsWith("JoinStringArray"))
                return $"{parametersList[0]}.Join({parametersList[1]})";
            if (functionName.StartsWith("Replace"))
                return $"{parametersList[0]}.Replace({parametersList[1]}, {parametersList[2]}, /* SearchCase = */ {parametersList[3]})";
            if (functionName.StartsWith("StartsWith"))
            ...
            if (functionName.StartsWith("IsNumeric"))
                return $"{parametersList[0]}.IsNumeric()";
            if (functionName.StartsWith("Len"))
        ...
        if (className == "KismetSystemLibrary")
        {
            ...
            if (functionName.StartsWith("Conv_ObjectToSoftObjectReference") || functionName.StartsWith("Conv_SoftObjPathToSoftObjRef"))
                return $"TSoftObjectPtr<UObject>({parametersList[0]})";
            if (functionName.StartsWith("Delay") && parametersList.Count == 3)
                return $"Delay({parametersList[1]}f);\n{parametersList[2]}";
            if (functionName.StartsWith("Conv_SoftClassPathToSoftClassRef"))
                return $"TSoftClassPtr<UObject>({parametersList[0]})";
            if (functionName.StartsWith("Conv_ClassToSoftClassReference"))
                return $"TSoftClassPtr<UObject>(*{parametersList[0]})";
        }
        if (className == "KismetInputLibrary" || className == "BlueprintGameplayTagLibrary" || className == "FortKismetLibrary" || className == "KismetTextLibrary")
        {
            ...
            if (functionName.StartsWith("NotEqual_"))
                return $"({parametersList[0]} != {parametersList[1]})";
            if (functionName.EndsWith("ToText"))
        ...
        string parameters)
    {
        ...
        {
            if (functionName.StartsWith("Array_Length"))
                return $"{parametersList[0]}.Length";
            if (functionName.StartsWith("Array_IsNotEmpty"))
                return $"{parametersList[0]}.Length > 0";
            if (functionName.StartsWith("Array_LastIndex"))
                return $"{parametersList[0]}.Length - 1";
            if (functionName.StartsWith("Array_Clear"))
                return $"{parametersList[0]}.Clear()";
            if (functionName.StartsWith("Array_Identical"))
                return $"{parametersList[0]} == {parametersList[1]}";
            if (functionName.StartsWith("Array_Remove"))
                return $"{parametersList[0]}.Remove({parametersList[1]})";
            if (functionName.StartsWith("Array_Add"))
                return $"{parametersList[0]}.Add({parametersList[1]})";
            if (functionName.StartsWith("Array_Get"))
                return $"{parametersList[2]} = {parametersList[0]}[{parametersList[1]}]";
            if (functionName.StartsWith("Array_Contains"))
                return $"{parametersList[0]}[{parametersList[1]}]";
            if (functionName.StartsWith("Array_IsValidIndex"))
                return $"{parametersList[0]}[{parametersList[1]}]";
            if (functionName.StartsWith("Array_Insert"))
                return $"{parametersList[0]}[{parametersList[2]}] = {parametersList[1]}";
        }

        if (className == "BlueprintMapLibrary")
        {
            if (functionName.StartsWith("Map_Length"))
                return $"{parametersList[0]}.Length";
            if (functionName.StartsWith("Map_Remove"))
                return $"{parametersList[0]}.Remove({parametersList[1]})";
            if (functionName.StartsWith("Map_Contains"))
                return $"{parametersList[0]}[{parametersList[1]}]";
        ...
        {
            if (functionName.StartsWith("Set_AddItems"))
                return $"{parametersList[0]}.Add({parametersList[1]})";
            if (functionName.StartsWith("Set_Clear"))
                return $"{parametersList[0]}.Clear()";
            if (functionName.StartsWith("Set_Difference"))
                return $"{parametersList[2]} = {parametersList[0]} == {parametersList[1]}";
            if (functionName.StartsWith("Set_IsEmpty"))
                return $"{parametersList[0]}.Length == 0";
        }
            ...
            case EX_FinalFunction final:
                {
                    ...
                    else
                    {
                        ...
                        if (final.Parameters.Length > 2 && final.Parameters[2] is EX_StructConst)
                        {
                            var structConst = final.Parameters[2] as EX_StructConst;
                            ...
                            if (structConst.Struct.Name == "LatentActionInfo")
                                return $"U{classType}::{functionName}({GetLineExpression(final.Parameters[1])});\r\n\r\n{properties[0]}";
                        }
            ...
            case EX_BindDelegate bindDelegate:
                {
                    ...
                    return $"{delegateVar} = CreateEvent({objectTerm}, {functionName})";
                }
            case EX_StructConst structConst:
                {
                    ...
                    if (structConst.Struct.Name == "LatentActionInfo")
                        return properties.Count > 0 ? properties[0] + ";" : ""; // used for cleaning code output.
            ...
            case EX_AddMulticastDelegate multicastDelegate:
                {
                    ...
                    return $"{delegatee}->Bind({delegateToAdd})";
                }
            ...
            case EX_CallMulticastDelegate callMulticastDelegate:
                {
                    ...
                    return $"{callDelegate}->Call({parametersString})";
                }
            case EX_RemoveMulticastDelegate removeMulticastDelegate:
                {
                    ...
                    return $"{delegateTarget}{separator}Unbind({delegateToRemove})";
                }
